...
Verify you have the connectivity with the Kubernete cluster and the Kubernetes API by by using the CLI.

`` kubectl cluster-info --context kind-kind``

The output displays the IPD adress
...
Now that the application, web, is deployed you can access the application by exposing the nodePort through port forwarding. <<However, to port forward the container port the container name is required.>>
